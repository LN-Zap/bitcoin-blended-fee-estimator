...
export const ESPLORA_FALLBACK_BASE_URL = config.get<string>('esplora.fallbackBaseUrl');

export const MEMPOOL_BASE_URL = config.get<string>('mempool.baseUrl');
export const MEMPOOL_FALLBACK_BASE_URL = config.get<string>('mempool.fallbackBaseUrl');
...
export const BITCOIND_CONF_TARGETS = config.get<number[]>('bitcoind.confTargets');
export const BITCOIND_ESTIMATE_MODE = config.get<EstimateMode>('bitcoind.estimateMode');
...
// Constants
export const MEMPOOL_TIP_HASH_URL = MEMPOOL_BASE_URL && `${MEMPOOL_BASE_URL}/api/blocks/tip/hash`;
export const ESPLORA_TIP_HASH_URL = ESPLORA_BASE_URL && `${ESPLORA_BASE_URL}/api/blocks/tip/hash`;
export const MEMPOOL_FEES_URL = MEMPOOL_BASE_URL && `${MEMPOOL_BASE_URL}/api/v1/fees/recommended`;
export const ESPLORA_FEE_ESTIMATES_URL = ESPLORA_BASE_URL && `${ESPLORA_BASE_URL}/api/fee-estimates`;

export const MEMPOOL_TIP_HASH_URL_FALLBACK = MEMPOOL_FALLBACK_BASE_URL && `${MEMPOOL_FALLBACK_BASE_URL}/api/blocks/tip/hash`;
export const ESPLORA_TIP_HASH_URL_FALLBACK = ESPLORA_FALLBACK_BASE_URL && `${ESPLORA_FALLBACK_BASE_URL}/api/blocks/tip/hash`;
export const MEMPOOL_FEES_URL_FALLBACK = MEMPOOL_FALLBACK_BASE_URL && `${MEMPOOL_FALLBACK_BASE_URL}/api/v1/fees/recommended`;
export const ESPLORA_FEE_ESTIMATES_URL_FALLBACK = ESPLORA_FALLBACK_BASE_URL && `${ESPLORA_FALLBACK_BASE_URL}/api/fee-estimates`;
 ...
 */
export function getValueFromFulfilledPromise(result: PromiseSettledResult<any>) {
...
export async function fetchWithTimeout(url: string, timeout: number = TIMEOUT): Promise<Response> {
 ...
 */
export async function fetchAndProcess(url: string, expectedResponseType: ExpectedResponseType): Promise<string | object | null> {
 ...
 */
export async function fetchAndHandle(url: string, expectedResponseType: ExpectedResponseType, fallbackUrl?: string): Promise<string | object | null> {
  try {
...
export async function fetchBitcoindData() : Promise<FeeByBlockTarget | null> {
  ...
  return new Promise((resolve, _) => {
    ...
    rpc.batch(batchCall, (error: Error | null, response: BitcoindRpcBatchResponse[]) => {
      if (error) {
        log.error({ message: 'Unable to fetch fee estimates from bitcoind: {error}', error });
        resolve(null);
      } else {
        targets.forEach((target, i) => {  
          ...
          if (feeRate) {
            ...
            log.error({ message: `Failed to fetch fee estimate from bitcoind for confirmation target ${target}: {errors}`,
...
export function processEstimates(estimates: FeeByBlockTarget, applyMultiplier = true, convert = false) : FeeByBlockTarget {
 ...
 */
export function extractMempoolFees(mempoolFeeEstimates: MempoolFeeEstimates): FeeByBlockTarget {
 ...
 * Calculates the fees.
 */
export function calculateFees(mempoolFeeEstimates: MempoolFeeEstimates, esploraFeeEstimates: FeeByBlockTarget, bitcoindFeeEstimates: FeeByBlockTarget) {
