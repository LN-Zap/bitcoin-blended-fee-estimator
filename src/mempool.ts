...
export class MempoolProvider implements Provider {
     ...
     */
    async getFeeEstimates(maxDepth: number = this.depth): Promise<FeeByBlockTarget> {
        const data = await this.fetchData<MempoolFeeEstimates>('/api/v1/fees/recommended', 'json');

        if (!data.fastestFee || !data.halfHourFee || !data.hourFee) {
            throw new Error("Invalid fee data");
        }

        const feeEstimates: FeeByBlockTarget = this.transformFeeData(data, maxDepth);
        return feeEstimates;
    }
    ...
    async getBlockHeight(): Promise<number> {
        const height = await this.fetchData<string>('/api/blocks/tip/height', 'text');
     ...
     */
    private async fetchData<T>(endpoint: string, responseType: 'json' | 'text'): Promise<T> {
        try {
            const response = await fetchWithTimeout(`${this.url}${endpoint}`, this.timeout);
